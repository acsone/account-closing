...
class AccountMove(models.Model):
    ...
    def _get_accrual_move_line_vals(self, account, move_prefix, move_line_prefix):
        ...
        if self.invoice_payment_term_id:
            totlines = self.invoice_payment_term_id._compute_terms(
                self.amount_untaxed, self.invoice_date or False
            )
        if totlines:
            totlines = totlines[0]
            res_amount_currency = self.amount_untaxed
            i = 0
            for t in totlines:
                if self.currency_id.id != company_currency.id:
                    amount_currency = company_currency.with_context(
                        date=self.date_invoice
                    ).compute(t[1], self.currency_id)
                else:
        ...
        return {
            ...
            "amount_currency": self.amount_untaxed_signed,
    ...
    def _get_accrual_move_vals(
        ...
        move_lines.append(
            self._get_accrual_move_line_vals(account, move_prefix, move_line_prefix)
        )
        return {
            "ref": accrual_ref,
