...
class AccountCutoff(models.Model):
    ...
    def _get_taxes(self, line):
        ...
        if self.cutoff_type == "accrued_expense":
            return fpos.map_tax(line.taxes_id)
        return fpos.map_tax(line.tax_id)

    def _get_received_qty(self, line, cutoff_nextday):
        if self.cutoff_type == "accrued_expense":
            received_qty = line.qty_received
            # Processing purchase order line
            # analytic_account = self._get_expense_analytic(line) FIXME
    ...
    def _prepare_line(self, line):
        ...
        accrual_account_id = self._get_mapping_dict().get(account_id, account_id)
        ...
        res = {
            ...
            # "analytic_account_id": analytic_account.id, # FIXME
            "currency_id": line.currency_id.id,
            "price_unit": price_unit,
            "tax_line_ids": [(6, 0, [tax.id for tax in taxes])],
